 ...
 *
 * The @param list is derived from the same strategy used by
 * buildMethodParams() so the docblock always matches the real signature.
 */
    ...
    switch ($m->paramStrategy) {
        ...
        case "explicit":
            ...
            foreach ($m->queryParams as $p) {
                $lines[] = " * @param " . phpType($p["type"]) . " \${$p["name"]}";
            }
            break;
    }
 ...
 * Removes stale model files that no longer have a matching schema.
 */
function generateModels(array $spec, string $outputDir): array
{
    ...
    // Remove stale files left from older spec versions
    foreach (glob($outputDir . "/*.php") as $existing) {
        $name = basename($existing, ".php");
...
function buildModelClass(
    ...
    $lines[] = "";
    $lines[] = "/**";
    $lines[] = " * 🤖 Generated by scripts/generate.php from the OpenAPI spec — do not edit.";
    $lines[] = " * Regenerate with: make generate";
    $lines[] = " */";
    $lines[] = "";
    $lines[] = "namespace Clearsoft\\EasySQL\\Client\\Models;";
    $lines[] = "";
    $lines[] = "class {$name}";
    $lines[] = "{";
